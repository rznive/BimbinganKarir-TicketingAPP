<x-layouts.admin title="Manajemen Tiket">
    ...
    <div class="container mx-auto p-10">
        <div class="flex">
            ...
            <button class="btn btn-primary ml-auto" onclick="add_modal.showModal()">Tambah Tiket</button>
        </div>
        <div class="overflow-x-auto rounded-box bg-white p-5 shadow-xs">
            <table class="table">
                ...
                <thead>
                    <tr>
                        <th>No</th>
                        <th class="w-3/4">Nama Tiket</th>
                        <th>Aksi</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($ticketTypes as $index => $category)
                        <tr>
                            <th>{{ $index + 1 }}</th>
                            <td>{{ $category->name }}</td>
                            <td>
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="text-center">Tidak ada Tiket tersedia.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <dialog id="add_modal" class="modal">
        <form method="POST" action="{{ route('admin.ticket_types.store') }}" class="modal-box">
            @csrf
            <h3 class="text-lg font-bold mb-4">Tambah Tiket</h3>
            <div class="form-control w-full mb-4">
                <label class="label mb-2">
                    <span class="label-text">Nama Tiket</span>
                </label>
                <input type="text" placeholder="Masukkan nama Tiket" class="input input-bordered w-full"
                    name="name" required />
            </div>
    ...
    <dialog id="edit_modal" class="modal">
        <form method="POST" class="modal-box">
            ...
            <h3 class="text-lg font-bold mb-4">Edit Tiket</h3>
            <div class="form-control w-full mb-4">
                <label class="label mb-2">
                    <span class="label-text">Nama Tiket</span>
                </label>
                <input type="text" placeholder="Masukkan nama Tiket" class="input input-bordered w-full"
                    value="Tiket Contoh" id="edit_category_name" name="name" />
            </div>
            <div class="modal-action">
                <button class="btn btn-primary" type="submit">Simpan</button>
    ...
    <dialog id="delete_modal" class="modal">
        <form method="POST" class="modal-box">
            ...
            <h3 class="text-lg font-bold mb-4">Hapus Tiket</h3>
    ...
    <script>
        function openEditModal(button) {
            ...
            form.action = `/admin/ticket_types/${id}`
